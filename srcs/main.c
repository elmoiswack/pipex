...
/*   Updated: 2023/04/03 14:44:23 by dhussain         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_process_1(char *argv[], char *envp[], int *pipes, t_struct *strct)
{
	...
	if (fd < 0)
		error_fork(pipes, 0, 1);
	if (dup2(fd, STDIN_FILENO) == -1)
		error_fork(pipes, fd, 1);
...
void	pipe_exec(char *argv[], char *envp[], t_struct *strct)
{
	...
	waitpid(child1, &wait_status_1, 0);
	close(pipes[0]);
	close(pipes[1]);
}
